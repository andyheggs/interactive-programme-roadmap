...
export async function exportExecutiveRoadmapPdf({ schedule, tracker, dateWindow, contextItemUids = [], removedItemUids = [], laneOrderUids = [] }: ExportExecutiveRoadmapOptions) {
  ...
  outcomes.forEach((outcome) => {
    ...
    const rows = [...dependencies, outcome].map((item) => {
      ...
      return [
        formatDate(item.finishDate),
        item.uid === outcome.uid ? "Executive outcome" : contextUidSet.has(item.uid) ? "Added context" : "Route milestone",
        executiveToneLabel(item),
        assessExecutiveTone(item).summary,
        item.name,
        direct || "-",
